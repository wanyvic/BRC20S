...
pub fn withdraw_user_reward(
  ...
  debug!("  pending reward:{}", pending_reward);

  debug!("  {}", user);
  debug!("  {}", pool);
  debug!("withdraw_user_reward out");
  pending_reward.truncate_to_u128()
}
...
fn get_per_share_multiplier() -> Num {
  get_num_by_decimal(PER_SHARE_MULTIPLIER).unwrap()
}

fn get_num_by_decimal(decimal: u8) -> Result<Num, BRC20SError> {
  BIGDECIMAL_TEN.checked_powu(u64::from(decimal))
}
...
mod tests {
  ...
  fn test_hello() {
    ...
    let erate = erate_base;
    let dmax = 100 * erate_base;

    let pid = Pid::from_str("Bca1DaBca1D#1").unwrap();
    let mut pool = new_pool(&pid, PoolType::Fixed, erate, dmax);
  ...
  fn test_complex_pool_one_user() {
    ...
    let pid = Pid::from_str("Bca1DaBca1D#1").unwrap();
    let mut pool = new_pool(&pid, PoolType::Pool, erate, dmax);
    let mut user = new_user(&pid);
  ...
  fn test_unknown_pool() {
    ...
    let pid = Pid::from_str("Bca1DaBca1D#1").unwrap();
    let mut pool = new_pool(&pid, PoolType::Unknown, erate, dmax);
    let mut user = new_user(&pid);
  ...
  fn do_one_case(
    ...
    match result {
      Ok(value) => {
        assert_eq!(value, expect_withdraw_reward_result.unwrap());
      }
      Err(err) => {
        println!("err:{:?}", err);
        assert_eq!(err, expect_withdraw_reward_result.unwrap_err())
      }
    }
    ...
    match u_result {
      ...
      Err(err) => {
        println!("err:{:?}", err);
        assert_eq!(err, expect_update_stake_result.unwrap_err())
      }
    }
